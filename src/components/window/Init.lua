...
return function(Config)
	local Window = {
		...
		ShadowTransparency = Config.ShadowTransparency or 0.05,
		User = Config.User or {},
	...
	function Window:Open()
		...
		task.spawn(function()
			...
			-- Leviathan: smooth sequential open
			-- Step 1: reset to collapsed state
			Window.UIElements.Main.Size = UDim2.new(Window.Size.X.Scale, Window.Size.X.Offset, 0, 0)
			if not Window.UIElements.Main:FindFirstChild("LevScale") then
				local sc = Instance.new("UIScale"); sc.Scale = 0.94; sc.Name = "LevScale"
				sc.Parent = Window.UIElements.Main
			end
			Window.UIElements.Main.LevScale.Scale = 0.94

			-- Step 2: expand height (Back.Out — gentle overshoot, clean)
			local openTween = Tween(Window.UIElements.Main, 0.42, {
				Size = Window.Size,
			}, Enum.EasingStyle.Back, Enum.EasingDirection.Out)
			openTween:Play()

			-- Step 3: scale in concurrently, slight delay for sequential feel
			task.delay(0.04, function()
				Tween(Window.UIElements.Main.LevScale, 0.36, {
					Scale = 1,
				}, Enum.EasingStyle.Quint, Enum.EasingDirection.Out):Play()
			end)
	...
	function Window:Close()
		...
		-- Leviathan: clean sequential close
		-- Step 1: subtle scale down first
		if Window.UIElements.Main:FindFirstChild("LevScale") then
			Tween(Window.UIElements.Main.LevScale, 0.18, {
				Scale = 0.96,
			}, Enum.EasingStyle.Quint, Enum.EasingDirection.In):Play()
		end
		-- Step 2: collapse height after brief delay
